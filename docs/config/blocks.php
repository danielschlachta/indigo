<?php

$font = 'font-family: georgia,serif; font-size: 120%; line-height: 1.2;';

$view = new idg_view;
$view->set_properties(array(
    'class' => 'fixedcontent',
	'name' => 'blocks',
	'icon' => 'favicon.png',
	'style' => $font . ' background: #53538a ' . 'url(elements/blocks/bg_grad.png) ' . 'fixed repeat-x;'
));

$centercol = new idg_container;
$centercol->set_properties(array(
	'name' => 'center',
	'style' => 'position: relative; top: 0; left: 0; margin-left: 130px; margin-right: 330px; z-index: 201;',
	'style-print' => 'margin: 0;'
));

$view->add_child($centercol);

$tabs = new idg_outlet;
$tabs->set_properties(array(
	'class' => 'blocks_tabs',
	'slot' => '_site',
    'tag' => 'main'
));
$centercol->add_child($tabs);

$style_link = 'text-decoration: none; color: black; background: url(elements/blocks/underline.png) bottom left repeat-x; padding-bottom: 1px; padding-right: 1px; margin-right: -1px; white-space: nowrap;';
$style_link_hover = 'background: #729e8d url(elements/blocks/underline.png) bottom left repeat-x;';

$centercol_body = new idg_container;
$centercol_body->set_properties(array(
	'name' => 'center_body',
	'style' => 'height: 100%; border: solid #1e3723; border-width: 0px 1px 1px 1px; padding: 0 30px 30px 23px; background: #b4d2b0 url(elements/blocks//line_red.png) repeat-y; text-align: justify;',
	'style-h1' => 'padding: 0.35em 0.5em 0.35em 0; border: solid #729e8d; border-width: 2px 0 2px 0; background: url(elements/blocks/randomsymbols_bg.jpg); margin: 0 0 0.5em 0; text-align: right; font-weight: bold; font-style: italic; font-size: 150%;',
	'style-h2' => 'padding: 0.1em 1em 0 0; border: solid #729e8d; border-width: 1px 0 1px 0; background: url(elements/blocks/randomsymbols_bg.jpg); margin: 0; text-align: right; font-weight: normal; font-style: italic; font-size: 120%;',
	'style-img' => 'border: 0;',
	'style-print' => 'border: 0; padding: 1em; margin: 100px 0 0 0;',
	'filter' => 'blocks_filter_linkimg'
));

$centercol->add_child($centercol_body);

$main_text = new idg_slot;
$main_text->set_properties(array(
	'name' => 'main-text',
	'style' => 'padding: 0 0 0 17px; background: url(elements/blocks/line_blue.png); font-family: Garamond, Georgia, \'DejaVu Serif\', serif;',
	'style-a' => $style_link,
	'style-a-hover' => $style_link_hover,
	'style-list-img' => 'elements/blocks/bullet.png',
	'filter' => 'blocks_filter_blockquote'
));
$centercol_body->add_child($main_text);

$footer_container = new idg_container;
$footer_container->set_properties(array(
	'name' => 'footer-container',
	'style' => "background: #b4d2b0 "
		. "url($elements/footer/line_red.png) repeat-y;"
		. "padding: 15px 25px 0 35px; margin: 6px 1px  1px;",
	'filter' => 'blocks_filter_linkimg'
));
$centercol->add_child($footer_container);

$footer = new idg_outlet;
$footer->set_properties(array(
	'class' => 'blocks_footer',
	'name' => 'footer',
	'tag' => 'Copyright &copy; 2023 Daniel Schlachta',
	'style' => "background-image: url($elements/footer/metal.png); "
		. " $font_blocks; font-size: 70%;  padding: 5px;",
	'style-a' => $style_link . ' height: 1.2em;',
	'style-a-hover' => $style_link_hover,
    'style-img' => 'margin-bottom: -1px; border: 0;'
));
$footer_container->add_child($footer);

$logo = new idg_container;
$logo->set_properties(array(
	'name' => 'logo',
	'style' => 'position: fixed; text-align: right; top: 20px; right: 20px; z-index: 120;',
	'style-print' => 'position: absolute; top: 0; right: 0;'
));

$logo_text = new idg_fragment;
$logo_text->set_properties(array(
    'class' => 'idg_fragment_text',
	'style' => 'padding-right: 8px; font-family: verdana, sans-serif; color: #394a71; font-style: italic;'
));
$logo_text->set_text('<b>indigo</b> / web');

$logo->add_child($logo_text);

$logo_image = new idg_fragment;
$logo_image->set_properties(array(
	'class' => 'image'
));

$image = $logo_image->create_attribute('image');
$image->set_parameter('src', 'elements/blocks/indigo.png');

$logo->add_child($logo_image);

$view->add_child($logo);

$imageframe = new idg_outlet;
$imageframe->set_properties(array(
	'class' => 'blocks_imageframe'
));

$attr = $imageframe->create_attribute('image');
$attr->set_parameter('top', 30);
$attr->set_parameter('left', 30);

$view->add_child($imageframe);

$infobox = new idg_outlet;
$infobox->set_properties(array(
	'class' => 'blocks_infobox'
));
$view->add_child($infobox);

$dropdown = new idg_outlet;
$dropdown->set_properties(array(
	'class' => 'blocks_dropdown',
	'slot' => '_site',
	'tag' => 'resources',
	'style' => 'position: fixed; top: 160px; right: 30px;'
));
$attr = $dropdown->create_attribute('dropdown');
$attr->set_parameter('bg-url', 'elements/blocks/compass.png');

$view->add_child($dropdown);

$searchbox = new idg_fragment;
$searchbox->set_properties(array(
	'class' => 'searchbox'
));

$view->add_child($searchbox);

$toolbox = new idg_fragment;
$toolbox->set_properties(array(
	'class' => 'toolbox'
));

$view->add_child($toolbox);

$background_img = new idg_container;
$background_img->set_properties(array(
	'name' => 'image-bg'
));
$view->add_child($background_img);

$bg_sky_container = new idg_container;
$bg_sky_container->set_properties(array(
	'name' => 'image-sky',
	'style' => 'position: fixed; top: 49px; right: 212px; width: 629px; height: 80px; background: url(elements/blocks/bg_sky.png) no-repeat; z-index: 101;'
));

$background_img->add_child($bg_sky_container);

$bg_left = new idg_container;
$bg_left->set_properties(array(
	'name' => 'image-graph-l',
	'style' => 'position: fixed; top: 352px; left: 0px; ' . 'width: 130px; height: 75px; ' . 'background: url(elements/blocks/bg_graph_l.png) no-repeat; z-index: 101;'
));

$background_img->add_child($bg_left);

$bg_right = new idg_container;
$bg_right->set_properties(array(
	'name' => 'image-graph-r',
	'style' => 'position: fixed; top: 352px; right: 0px; ' . 'width: 330px; height: 130px; ' . 'background: url(elements/blocks/bg_graph_r.png) no-repeat; z-index: 101;'
));

$background_img->add_child($bg_right);

?>