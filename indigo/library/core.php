...
class idg_core {
    ...
    /**
     * Constructs an idg_core object and possibly loads a site template.
     ...
     * subsequently used throughout the idg_core object.<br>
     ...
     * @param string $template_name The entry point without the <code>.php</code> suffix
     * @param string $template_path The path to where <code>php</code> can find the file
     * @param string $template_uri The path part of the <code>URL</code> if it differs
     */
    public function __construct(string $template_name = null,
        string $template_path = null, string $template_uri = null) {
    ...
    function get_template_path(): ?string {
        return $this->template_path ? $this->template_path : $this->template_name;
    }
     ...
     */
    function get_template_uri(string $subfolder = null): string {
        $uri = $this->template_uri ? $this->template_uri : $this->get_template_path();
    ...
    private function load_template(string $template_name): void {
        ...
        $filename = $this->get_template_path() . "/$template_name.php";
        ...
        if (preg_match('/.*namespace[ \t\n]+([^;]+);/', $string, $matches) !== 1)
            idg_log('core/load template: no namespace found in template');
        else
